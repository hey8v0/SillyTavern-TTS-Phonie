...
import {
    ...
    getLauncherMode as getLauncherModeModule,
    mountPhonieLaunchers as mountPhonieLaunchersModule,
} from './shell.js';
import {
    markStickerStatus,
                ...
                <div class="voice-drawing-params">
                    <label class="voice-field" for="tts-drawing-model">
                        ...
                        <select id="tts-drawing-model" name="model">
                            ${NOVELAI_MODELS.map(item => `<option value="${item.id}" ${(params.model || 'nai-diffusion-3') === item.id ? 'selected' : ''}>${item.label}</option>`).join('')}
                        </select>
                    </label>
...
function renderSettingsHome() {
    ...
    const entries = [
        ...
        { tab: 'prompts', icon: 'layers', name: '全部提示词', note: '正文、单人通话、多人通话、私聊、群聊与生图六类预设' },
        ...
        <section class="voice-secondary-view" aria-labelledby="voice-settings-heading">
            ...
            <form id="tts-planner-settings-form" class="voice-planner-card" data-planner-settings-form>
                ...
                <div class="voice-planner-custom" data-planner-custom ${planner.mode === 'custom' ? '' : 'hidden'}>
                    ...
                    <label class="voice-field" for="tts-planner-response-mode">
                        <span class="voice-field-label">响应方式</span>
                        <select id="tts-planner-response-mode" name="responseMode">
                            ${RESPONSE_MODE_OPTIONS.map(item => `<option value="${item.id}" ${planner.responseMode === item.id ? 'selected' : ''}>${item.label}</option>`).join('')}
                        </select>
                        <small>旧预设默认非流式；只有接口支持 SSE 时才选择流式。</small>
                    </label>
            ...
            <form id="tts-display-settings-form" class="voice-route-editor" data-display-settings-form>
                ...
                <label class="voice-field" for="tts-launcher-mode">
                    ...
                    <select id="tts-launcher-mode" name="launcherMode">
                        <option value="orb" ${getLauncherModeModule({ TTS_ProviderRegistry }) === 'orb' ? 'selected' : ''}>悬浮球</option>
                        <option value="wand" ${getLauncherModeModule({ TTS_ProviderRegistry }) === 'wand' ? 'selected' : ''}>酒馆魔棒菜单</option>
                        <option value="both" ${getLauncherModeModule({ TTS_ProviderRegistry }) === 'both' ? 'selected' : ''}>两者都显示</option>
                    </select>
...
function renderSettingsQqPage() {
    ...
    return `
        <section class="voice-secondary-view" aria-labelledby="voice-settings-heading">
            ...
            <button type="button" class="voice-route-shortcut" data-open-prompt-workflow="group_chat">${icon('users', 18)}<span><strong>群聊提示词</strong><small>单独控制群成员出场、接话与沉默</small></span>${icon('chevronRight', 16)}</button>
...
function renderSettingsStickersPage() {
    const qq = TTS_ProviderRegistry.getQqState();
    const stickers = normalizeStickerLibrary(qq.stickers);
            ...
            <section id="tts-settings-stickers" class="voice-route-editor" aria-label="表情包">
                ...
                <div class="voice-sticker-grid" data-preserve-scroll="sticker-grid">
                    ${stickers.length ? stickers.map(sticker => {
                        ...
                        const catboxUnavailable = sticker.status === 'failed' && /catbox\.moe/i.test(sticker.url);
                        return `<figure class="voice-sticker-item state-${safe(sticker.status)} ${selected.has(sticker.id) ? 'is-selected' : ''}">
                            ...
                            <img src="${safe(sticker.url)}${state.stickerRetryNonce ? `${sticker.url.includes('?') ? '&' : '?'}phonie_retry=${state.stickerRetryNonce}` : ''}" alt="${safe(sticker.name || '表情包')}" loading="lazy" data-sticker-image="${safe(sticker.id)}">
                            ${isEditing ? `
                            ...
                            <figcaption><span>${safe(sticker.name || '未命名')}<small>${sticker.status === 'ready' ? '链接正常' : sticker.status === 'failed' ? (catboxUnavailable ? '图床不可达' : '图片加载失败') : '等待检查'}</small></span>
                                <span class="voice-sticker-item-actions">
                                    ${sticker.status === 'failed' ? `<button type="button" data-sticker-retry="${safe(sticker.id)}" aria-label="重试加载">${icon('refresh', 13)}</button>` : ''}
...
async function runPhonePlan({ caller, brief, duration, direction = 'outgoing', participants = [] } = {}) {
    ...
    let plan = null;
    try {
        plan = await FrontendVoiceTools.generatePhonePlan({
            brief,
            duration,
            caller,
            participants,
        ...
        if (state.featureBusy === 'phone-plan') state.featureBusy = null;
        updateView();
    }
    // 必须在释放 featureBusy 之后再进入响铃，否则 answerPhoneCall 会被 busy guard 静默拦住。
    // 生成期间用户离开电话页（route 改变）则不响铃，避免幽灵呼叫。
    if (plan) beginOutgoingCall(plan, { requireRoute: 'incoming' });
}
...
function renderPhoneChatMessage(message, thread) {
    ...
    const sticker = message.type === 'sticker'
        ...
        : null;
    const stickerImage = message.type === 'sticker' && (sticker?.url || message.stickerUrl)
        ? `<img class="voice-chat-sticker-image" src="${safe(sticker?.url || message.stickerUrl)}" alt="表情包" data-chat-sticker-image data-chat-sticker-id="${safe(sticker?.id || message.stickerId || '')}">`
        : '';
        ...
        : message.type === 'transfer' ? `
            ...
            : message.type === 'sticker' ? `
                <div class="voice-chat-rich-card is-sticker">
                    ${stickerImage || `<span class="voice-chat-sticker-error">图片加载失败</span>`}
                </div>`
                : '';
    return `
        <li class="voice-chat-message ${sent ? 'is-sent' : 'is-received'} ${isVoice ? 'is-voice' : ''} ${actionsOpen ? 'is-actions-open' : ''}" data-chat-message-id="${safe(message.id)}">
            ${state.chatSelectionMode ? `<label class="voice-chat-select"><input type="checkbox" data-chat-select-message="${safe(message.id)}" ${(state.chatSelectedMessages || []).includes(message.id) ? 'checked' : ''}><i></i></label>` : ''}
            ...
            <div class="voice-chat-message-stack">
                <div class="voice-chat-bubble">
                    ...
                    ${missingQuote ? `<div class="voice-chat-quote-card is-missing"><span>原消息已删除</span></div>` : ''}
                    ${richContent || (isVoice ? `
...
function renderPhoneChat() {
    ...
    const context = chat.context || tools.context;
    ...
    const selectedMessages = new Set(state.chatSelectedMessages || []);
    const allMessagesSelected = messages.length > 0 && messages.every(message => selectedMessages.has(message.id));
    return `
        <section class="voice-secondary-view voice-chat-app" aria-labelledby="voice-chat-heading">
            ...
            <header class="voice-chat-contact">
                ...
                <div><strong>${safe(context.charName || '未选择角色')}</strong><small>${context.canGenerate === false ? '历史可查看 · 打开对应角色卡后可回复' : context.available ? '可聊天 · 使用当前角色卡与世界书' : '请先打开角色对话'}</small></div>
                <div class="voice-chat-header-actions">
                    <button class="voice-chat-call-button ${chatCallBusy ? 'is-busy' : ''}" type="button" data-start-chat-call aria-label="${chatCallBusy ? '正在生成语音通话' : `与 ${safe(context.charName || '角色')} 语音通话`}" ${!context.canGenerate || busy || chatCallBusy ? 'disabled' : ''}>
                    ...
                    <button type="button" data-toggle-chat-selection aria-label="${state.chatSelectionMode ? '完成消息管理' : '多选消息'}" aria-pressed="${state.chatSelectionMode}">${icon('check', 18)}</button>
                </div>
            </header>
            ${state.chatSelectionMode ? `<div class="voice-chat-bulk-toolbar"><label><input type="checkbox" data-chat-select-all ${allMessagesSelected ? 'checked' : ''}> 全选</label><span>已选 ${selectedMessages.size}/${messages.length}</span><button type="button" data-chat-delete-selected ${selectedMessages.size ? '' : 'disabled'}>${icon('trash', 14)} 删除</button></div>` : ''}
            ...
            <form class="voice-chat-composer" data-phone-chat-form data-pending-count="${pendingCount}" data-chat-available="${context.available}" ${state.chatSelectionMode ? 'hidden' : ''}>
                        ...
                        ${stickers.length
                            ? `<div class="voice-chat-sticker-grid">
                                ${stickers.map(sticker => `<label class="voice-chat-sticker-item">
                                    <input type="radio" name="stickerName" value="${safe(sticker.name)}" data-sticker-id="${safe(sticker.id)}" data-sticker-url="${safe(sticker.url)}">
...
function renderPanel() {
    if (state.route === 'incoming') return renderIncomingView({
        FrontendVoiceTools,
        state,
        safe,
        icon,
...
function renderRoute() {
    ...
    if (state.route === 'qq') return renderQqView({
        FrontendVoiceTools,
    ...
    if (state.route === 'contacts') return renderContactsView({ FrontendVoiceTools, state, safe, icon });
...
async function startPhoneChatCall() {
    ...
    try {
        ...
        if (state.featureBusy === 'chat-call') state.featureBusy = null;
        updateView();
    }
    // 生成期间用户离开聊天页（route 改变）则不再响铃，避免幽灵呼叫。
    if (plan) beginOutgoingCall(plan, { requireRoute: 'chat' });
}
...
function collectPlannerForm(form) {
    ...
    const values = {
        ...
        responseMode: form.elements.responseMode?.value ?? planner.responseMode,
...
function getPhoneChatComposerState(form) {
    ...
    const selectedSticker = form?.querySelector('input[name="stickerName"]:checked');
    const stickerId = selectedSticker?.dataset?.stickerId || '';
    const stickerUrl = selectedSticker?.dataset?.stickerUrl || '';
    const sendable = (() => {
        if (text) return true;
...
function bindEvents(eventRoot) {
    eventRoot.addEventListener('load', event => {
        if (event.target?.hasAttribute?.('data-chat-sticker-image')) {
            const chatStickerId = event.target.dataset.chatStickerId;
            if (chatStickerId) {
                const qq = TTS_ProviderRegistry.getQqState();
                TTS_ProviderRegistry.updateQqState({ stickers: markStickerStatus(qq.stickers, chatStickerId, 'ready') });
            }
            return;
        }
        const id = event.target?.dataset?.stickerImage;
        if (!id) return;
        const qq = TTS_ProviderRegistry.getQqState();
        const stickers = markStickerStatus(qq.stickers, id, 'ready');
        TTS_ProviderRegistry.updateQqState({ stickers });
    }, true);
    eventRoot.addEventListener('error', event => {
        if (event.target?.hasAttribute?.('data-chat-sticker-image')) {
            const id = event.target.dataset.chatStickerId;
            const url = event.target.currentSrc || event.target.src || '';
            const error = /catbox\.moe/i.test(url) ? '图床不可达' : '图片加载失败';
            if (id) {
                const qq = TTS_ProviderRegistry.getQqState();
                TTS_ProviderRegistry.updateQqState({ stickers: markStickerStatus(qq.stickers, id, 'failed', error) });
            }
            const fallback = document.createElement('span');
            fallback.className = 'voice-chat-sticker-error';
        ...
        if (!id) return;
        const qq = TTS_ProviderRegistry.getQqState();
        const sticker = normalizeStickerLibrary(qq.stickers).find(item => item.id === id);
        const error = /catbox\.moe/i.test(sticker?.url || '') ? '图床不可达' : '图片加载失败';
        TTS_ProviderRegistry.updateQqState({ stickers: markStickerStatus(qq.stickers, id, 'failed', error) });
        event.target.closest('.voice-sticker-item')?.classList.add('state-failed');
    }, true);
    eventRoot.addEventListener('click', async event => {
        ...
        if (event.target.closest('[data-toggle-chat-selection]')) {
            state.chatSelectionMode = !state.chatSelectionMode;
            state.chatSelectedMessages = [];
            state.chatActionId = '';
            updateView();
            return;
        }
        const chatSelectMessage = event.target.closest('input[data-chat-select-message]');
        if (chatSelectMessage) {
            const selected = new Set(state.chatSelectedMessages || []);
            if (chatSelectMessage.checked) selected.add(chatSelectMessage.dataset.chatSelectMessage);
            else selected.delete(chatSelectMessage.dataset.chatSelectMessage);
            state.chatSelectedMessages = [...selected];
            updateView();
            return;
        }
        const chatSelectAll = event.target.closest('input[data-chat-select-all]');
        if (chatSelectAll) {
            const messages = FrontendVoiceTools.getPhoneChatSnapshot().thread.messages || [];
            state.chatSelectedMessages = chatSelectAll.checked ? messages.map(message => message.id) : [];
            updateView();
            return;
        }
        if (event.target.closest('[data-chat-delete-selected]')) {
            const selected = [...new Set(state.chatSelectedMessages || [])];
            if (!selected.length) return;
            if (!window.confirm(`确定永久删除选中的 ${selected.length} 条消息吗？引用它们的消息会显示“原消息已删除”。`)) return;
            FrontendVoiceTools.deletePhoneChatMessages(selected);
            state.chatSelectedMessages = [];
            state.chatSelectionMode = false;
            state.chatQuoteId = '';
            updateView();
            announce(`已删除 ${selected.length} 条消息`);
            return;
        }
        ...
        const stickerRetry = event.target.closest('[data-sticker-retry]')?.dataset.stickerRetry;
        if (stickerRetry) {
            const qq = TTS_ProviderRegistry.getQqState();
            TTS_ProviderRegistry.updateQqState({ stickers: markStickerStatus(qq.stickers, stickerRetry, 'unchecked') });
            state.stickerRetryNonce = Date.now();
            updateView();
            announce('正在重新检查图片链接');
            return;
        }
        ...
        if (stickerSaveEdit) {
            ...
            const currentQq = TTS_ProviderRegistry.getQqState();
            const stickers = normalizeStickerLibrary(currentQq.stickers)
                .map(sticker => (sticker.id === id ? { ...sticker, name, url, status: 'unchecked', error: '' } : sticker));
            TTS_ProviderRegistry.updateQqState({ stickers });
            state.stickerEditingId = '';
            updateView();
            announce('表情包已更新');
        ...
        const qqSelectFriend = event.target.closest('input[data-qq-select-friend]');
        if (qqSelectFriend) {
            const selected = new Set(state.qqSelectedFriends || []);
            if (qqSelectFriend.checked) selected.add(qqSelectFriend.dataset.qqSelectFriend);
            else selected.delete(qqSelectFriend.dataset.qqSelectFriend);
            state.qqSelectedFriends = [...selected];
            updateView();
            return;
        }
        ...
        if (event.target.closest('[data-qq-delete-friends]')) {
            const selected = [...new Set(state.qqSelectedFriends || [])];
            if (!selected.length) return;
            const qq = TTS_ProviderRegistry.getQqState();
            const currentName = FrontendVoiceTools.getContextSnapshot().charName || '当前角色';
            ...
            updateView();
            announce(`已移除 ${selected.length} 位好友`);
            return;
        }
        ...
        if (qqOpenGroup) {
            ...
            state.groupSelectionMode = false;
            state.groupSelectedMessages = [];
            updateView();
            return;
        }
        ...
        if (event.target.closest('[data-toggle-group-selection]')) {
            state.groupSelectionMode = !state.groupSelectionMode;
            state.groupSelectedMessages = [];
            updateView();
            return;
        }
        const groupSelectMessage = event.target.closest('input[data-group-select-message]');
        if (groupSelectMessage) {
            const selected = new Set(state.groupSelectedMessages || []);
            if (groupSelectMessage.checked) selected.add(groupSelectMessage.dataset.groupSelectMessage);
            else selected.delete(groupSelectMessage.dataset.groupSelectMessage);
            state.groupSelectedMessages = [...selected];
            updateView();
            return;
        }
        const groupSelectAll = event.target.closest('input[data-group-select-all]');
        if (groupSelectAll) {
            const group = FrontendVoiceTools.getGroupChatSnapshot().groups.find(item => item.id === state.qqOpenGroup);
            state.groupSelectedMessages = groupSelectAll.checked ? (group?.messages || []).map(message => message.id) : [];
            updateView();
            return;
        }
        if (event.target.closest('[data-group-delete-selected]')) {
            const selected = [...new Set(state.groupSelectedMessages || [])];
            if (!selected.length || !state.qqOpenGroup) return;
            if (!window.confirm(`确定永久删除选中的 ${selected.length} 条群消息吗？引用它们的消息会显示“原消息已删除”。`)) return;
            FrontendVoiceTools.deleteGroupChatMessages(state.qqOpenGroup, selected);
            state.groupSelectedMessages = [];
            state.groupSelectionMode = false;
            updateView();
            announce(`已删除 ${selected.length} 条群消息`);
            return;
        }
        if (event.target.closest('[data-qq-group-reply]')) {
            if (state.featureBusy || !state.qqOpenGroup) return;
            state.featureBusy = 'group-chat-reply';
            updateView();
            try {
                const result = await FrontendVoiceTools.generateGroupChatReply(state.qqOpenGroup);
                announce(`群成员回复了 ${result.assistantMessages.length} 条消息`);
            } catch (error) {
                announce(error.message || '群成员回复失败');
            } finally {
                state.featureBusy = null;
                updateView();
            }
    ...
    eventRoot.addEventListener('submit', async event => {
        ...
        if (event.target.hasAttribute('data-sticker-add-form')) {
            ...
            const currentQq = TTS_ProviderRegistry.getQqState();
            const stickers = normalizeStickerLibrary(currentQq.stickers);
            const added = normalizeStickerLibrary(incoming);
        ...
        if (event.target.hasAttribute('data-sticker-bulk-form')) {
            ...
            const stickers = normalizeStickerLibrary(incoming);
            TTS_ProviderRegistry.updateQqState({ stickers });
        ...
        if (event.target.hasAttribute('data-qq-group-form')) {
            ...
            const group = FrontendVoiceTools.createGroupChat({ name: groupName, memberNames: members });
            state.qqGroupFormOpen = false;
            state.qqGroupDraft = null;
        ...
        if (event.target.hasAttribute('data-qq-group-message-form')) {
            event.preventDefault();
            const text = String(event.target.elements.message?.value || '').trim();
            if (!text) {
                announce('请先输入群消息');
                return;
            }
            try {
                const result = FrontendVoiceTools.appendGroupChatMessage(state.qqOpenGroup, { text });
                event.target.reset();
                updateView();
                announce(`群消息已发送，当前有 ${result.pendingCount} 条等待回复`);
            } catch (error) {
                announce(error.message || '群消息发送失败');
            }
            return;
        }
        ...
        if (event.target.hasAttribute('data-phone-plan-form')) {
            ...
            let participants;
            try {
                participants = readSelectedParticipants(event.target);
            } catch (error) {
        ...
        if (event.target.hasAttribute('data-display-settings-form')) {
            ...
            if (form.elements.launcherMode) {
                ...
                applyLauncherVisibilityModule({ TTS_ProviderRegistry });
            }
            updateView();
            announce('显示与语言设置已保存');
            return;
        ...
        if (event.target.hasAttribute('data-drawing-form')) {
            ...
            updateView();
            try {
                const response = await requestNovelAiImage({
                    prompt,
                    model: form.elements.model.value,
                    width,
                    height,
...
export const TTS_Mobile = {
    init() {
        ...
        const legacyQqGroups = TTS_ProviderRegistry.getQqState().groups || [];
        if (legacyQqGroups.length) {
            FrontendVoiceTools.migrateLegacyQqGroups(legacyQqGroups);
            TTS_ProviderRegistry.updateQqState({ groups: [] });
        }
        if (!savedUiSettings.theme) TTS_ProviderRegistry.updateUiSettings({ theme: state.theme });
        buildShell();
        mountPhonieLaunchersModule({ TTS_ProviderRegistry, TTS_Mobile });
        startClock();
        startSystemStatus();
        loadDrawingGallery();
        state.unsubscribe?.();
