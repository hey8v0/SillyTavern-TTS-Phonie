...
function readMotionColors(root) {
    ...
    return {
        cyan: normalize(styles.getPropertyValue('--phonie-signal').trim(), 'rgb(83, 196, 205)'),
        copper: normalize(styles.getPropertyValue('--phonie-emphasis').trim(), 'rgb(224, 150, 102)'),
        text: normalize(styles.getPropertyValue('--phonie-ink').trim(), 'rgb(244, 240, 232)'),
    };
}
...
/**
 * Interactive signal-rain curtain carried over from the original modified phone.
 * Pointer handlers only update input; canvas work stays in requestAnimationFrame
 * and pauses whenever the phone, home route, tab, or viewport is inactive.
 */
export function createPhoneMotionRuntime({ root, screen }) {
    ...
    let route = root.dataset.screen || 'home';
    let activity = 'idle';
    ...
    const attachCanvas = () => {
        const nextCanvas = screen.querySelector('[data-phonie-motion-canvas]');
